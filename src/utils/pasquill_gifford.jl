...
"""
    crosswind_dispersion(stability_class::Symbol; <keyword arguments>)
...
"""
function crosswind_dispersion(stability_class::Symbol; plume=true, avg_time=600)
    if stability_class ∈ [:A,:B,:C,:D,:E,:F]
        if plume
...
"""
vertical_dispersion(stability_class::Symbol, plume=true)
...
"""
function vertical_dispersion(stability_class::Symbol; plume=true)
    if stability_class ∈ [:A,:B,:C,:D,:E,:F]
        if plume
